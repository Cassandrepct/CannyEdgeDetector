...
public class CannyEdgeDetector {
     ...
     */
    private static BufferedImage makingThresholding(BufferedImage img) {
        int tMax = 40;
        int tMin = 20;
     ...
     */
    private static int[][] getSobelFilterX() {
     ...
     */
    private static int[][] getSobelFilterY() {
     ...
     */
    private static int[][] getSobelX(BufferedImage img, int[][] sobelFilterX) {
     ...
     * @return the final image
     */
    private static int[][] getSobelY(BufferedImage img, int[][] sobelFilterY) {
        int height = img.getHeight();
        int width = img.getWidth();
     ...
     */
    private static int[][] getGradientAngle(int[][] sobelX, int[][] sobelY, int[][] angle, int height, int width) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
     ...
     */
    private static int[][] getAverageGradientAngle(int [][]angle, int[][] averageAngle, int height, int width) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
     ...
     */
    private static BufferedImage deleteNonMaximum(BufferedImage img, int[][] averageAngle) {
        for (int y = 0; y < img.getHeight(); y++) {
            for (int x = 0; x < img.getWidth(); x++) {
     ...
     * @return the image treated
     */
    private static BufferedImage calculateMagnitude(BufferedImage img, int[][] sobelX, int[][] sobelY) {
        for (int y = 0; y < img.getHeight(); y++) {
            for (int x = 0; x < img.getWidth(); x++) {
                int magnitude = (int) Math.sqrt(Math.pow(sobelX[y][x], 2) + Math.pow(sobelY[y][x], 2));
     ...
     * @param img the image to treat
     */
    private static BufferedImage findGradients(BufferedImage img) {
        ...
        img = calculateMagnitude(img, sobelX, sobelY);
        System.out.println("OK.");

        System.out.println("Action: non-maximum suppression.. ");
        int[][] angle = new int[img.getHeight()][img.getWidth()];
        angle = getGradientAngle(sobelX, sobelY, angle, img.getHeight(), img.getWidth());

        int[][] averageAngle = new int[img.getHeight()][img.getWidth()];
        averageAngle = getAverageGradientAngle(angle, averageAngle, img.getHeight(), img.getWidth());

        img = deleteNonMaximum(img, averageAngle);
        System.out.println("OK.");
     ...
     */
    private static BufferedImage gaussianFilter(BufferedImage img) {
     ...
     */
    private static BufferedImage grayscale(BufferedImage myImage) {
     ...
     */
    private static void writeImage(BufferedImage myImage) {
        File myFile;
